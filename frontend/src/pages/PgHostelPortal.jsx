...
const PgHostelPortal = () => {
  ...
  useEffect(() => {
    const fetchPgListings = async () => {
      setLoading(true);
      try {
        const res = await api.get('/listings', { params: { type: 'pg', size: 30, _t: Date.now() } });
          ...
          <div className="text-center max-w-3xl mx-auto mb-12">
            ...
            <div className="inline-flex p-1.5 rounded-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-white/10 shadow-lg max-w-full overflow-x-auto">
              <button
                ...
                className={`px-6 py-3 rounded-full font-bold text-sm transition-all duration-300 flex items-center gap-2 whitespace-nowrap ${
                  activeTab === 'girls'
                    ...
                    : 'text-gray-600 dark:text-gray-400 hover:text-pink-600 dark:hover:text-pink-400 hover:bg-pink-50 dark:hover:bg-pink-950/30'
                }`}
              >
                👩 Girls PGs & Hostels
              </button>
              <button
                ...
                className={`px-6 py-3 rounded-full font-bold text-sm transition-all duration-300 flex items-center gap-2 whitespace-nowrap ${
                  activeTab === 'boys'
                    ? 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-md shadow-blue-500/25 scale-105'
                    : 'text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/30'
                }`}
              >
                👨 Boys PGs & Hostels
              </button>
